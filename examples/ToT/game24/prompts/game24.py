...
You want to explore all ways of combining the given numbers using '+', '-', '*', '/'. Continue doing so recursively until you either find a sequence of operations that results in exactly 24, referred to as optimal solution.
...
- Recursive exploration: After performing one operation, you now have fewer numbers. Repeat the process with the new set of numbers, continuing until:
    - You end up with a single number. If that number is 24, you have found a solution.
    - Or you have tried all possible sequences of operations without reaching 24.
...
- Format of steps:
    ...
    - Continue to show steps until you reach a single number. If that single number is 24, you will print out the entire sequence of steps that led there.
...
- Example leading to 24 (optimal solution is the way you are supposed to follow):
Input: 4 4 6 8
Steps:
4 + 8 = 12 (left: 4 6 12)
6 - 4 = 2 (left: 2 12)
...
- Example of optimal solution (The way you are supposed to follow):
Input: 2 9 10 12
Steps:
12 * 2 = 24 (left: 9 10 24)
10 - 9 = 1 (left: 1 24)
...
Important:
- You will not add any prefixes like "1.", "-" or other numbering before the operations.
- You will try not to have bias towards a specific operator while doing the operations. You will try to explore diverse solutions using diverse operators.
- Important: You must continue listing operations until you reach a single number of 24. Do not stop prematurely. Provide every arithmetic step that transforms the list of numbers until only one remains. No skipping.
